...
namespace Checkpoint.Data.Repositories
{
    public class ProductoRepository
    {
        ...
        public IEnumerable<Producto> GetAll()
        {
            ...
            using (var cmd = new SqlCommand(@"SELECT Id, Sku, Nombre, Unidad, VidaUtilDias, 
                                                     TempMin, TempMax, StockMinimo, Activo 
                                              FROM Producto ORDER BY Nombre", conn))
            {
                conn.Open();
        ...
        public void Insert(Producto p)
        {
            ...
            p.Id = Guid.NewGuid();
            using (var conn = new SqlConnection(_connectionString))
            using (var cmd = new SqlCommand(@"INSERT INTO Producto 
                        (Id, Sku, Nombre, Unidad, VidaUtilDias, TempMin, TempMax, StockMinimo, Activo) 
                 VALUES (@Id,@Sku,@Nombre,@Unidad,@VidaUtilDias,@TempMin,@TempMax,@StockMinimo,@Activo)", conn))
            {
        ...
        public void Update(Producto p)
        {
            if (p == null) throw new ArgumentNullException(nameof(p));
            using (var conn = new SqlConnection(_connectionString))
            using (var cmd = new SqlCommand(@"UPDATE Producto 
                 SET Sku=@Sku, Nombre=@Nombre, Unidad=@Unidad, VidaUtilDias=@VidaUtilDias, 
                     TempMin=@TempMin, TempMax=@TempMax, StockMinimo=@StockMinimo, Activo=@Activo 
                 WHERE Id=@Id", conn))
            {
                cmd.Parameters.AddWithValue("@Id", p.Id);
                cmd.Parameters.AddWithValue("@Sku", (object)p.Sku ?? DBNull.Value);
        ...
        /// <summary>
        /// Cantidad de productos cuyo stock total (sumado por lotes) está bajo StockMinimo.
        /// </summary>
        public int GetProductosBajoStockMinimoCount()
        {
            const string sql = @"
;WITH S AS (
    SELECT L.ProductoId, SUM(Sk.Cantidad) AS Qty
    FROM Stock Sk
    JOIN Lote L ON L.Id = Sk.LoteId
    GROUP BY L.ProductoId
)
SELECT COUNT(*)
FROM Producto P
LEFT JOIN S ON S.ProductoId = P.Id
WHERE P.Activo = 1
  AND ISNULL(S.Qty, 0) < P.StockMinimo;";
            using (var conn = new SqlConnection(_connectionString))
        ...
        /// <summary>
        /// Top N productos bajo stock mínimo (para la grilla de alertas).
        /// </summary>
        public IEnumerable<Producto> GetProductosBajoStockMinimo(int top = 10)
        {
            var list = new List<Producto>();
            string sql = $@"
;WITH S AS (
...
SELECT TOP({top}) P.Id, P.Sku, P.Nombre, P.Unidad, P.VidaUtilDias, 
       P.TempMin, P.TempMax, P.StockMinimo, P.Activo
FROM Producto P
LEFT JOIN S ON S.ProductoId = P.Id
WHERE P.Activo = 1
  AND ISNULL(S.Qty, 0) < P.StockMinimo
                ...
                using (var rdr = cmd.ExecuteReader())
                {
                    while (rdr.Read())
                    {
                        list.Add(new Producto
                        {
                            ...
                            Nombre = rdr.IsDBNull(2) ? null : rdr.GetString(2),
                            Unidad = rdr.IsDBNull(3) ? null : rdr.GetString(3),
                            VidaUtilDias = rdr.GetInt32(4),
                            TempMin = rdr.IsDBNull(5) ? (decimal?)null : rdr.GetDecimal(5),
                            TempMax = rdr.IsDBNull(6) ? (decimal?)null : rdr.GetDecimal(6),
                            StockMinimo = rdr.GetDecimal(7),
